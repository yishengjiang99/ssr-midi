...
var sampleDir = function (filename) { return path_1.resolve(__dirname, "../samples", filename); };
describe("ssrctx", function () {
    it("sets framerate, bitdepths etc", function () {
    ...
    it("stops on time", function (done) {
        var ctx = new ssrctx_1.SSRContext({
            nChannels: 2,
            bitDepth: 16,
            sampleRate: 44100,
    ...
    it("parse bitdepth from filename", function () {
        var ctx = ssrctx_1.SSRContext.fromFileName(sampleDir("song-f32le.pcm"));
        chai_1.expect(ctx.bitDepth).to.equal(32);
        chai_1.expect(ctx.sampleRate).to.equal(ssrctx_1.SSRContext.defaultProps.sampleRate);
    });
    it("writes sufficient amount of data for playback", function (done) {
        var ctx = new ssrctx_1.SSRContext({
            nChannels: 2,
            bitDepth: 32,
            sampleRate: 44100,
        });
        var fss = new audio_data_source_1.FileSource(ctx, {
