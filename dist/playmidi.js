...
exports.preCacheNotes = async (ctxstr, midiFile) => {
  ...
  const uniqNotes = parseInt(child_process_1.execSync(`cat ${midiFile} |cut -f1,2 -d',' |sort|uniq|wc -l`).toString().trim());
  const noteCache = new flat_cache_store_1.CacheStore(uniqNotes, ctx.bytesPerSecond * 2, `db/cache/${ctxstr}${midiFile}`);
  let notes = fs_1
